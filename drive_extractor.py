"""
HistorySnooze Safe Archive Extraction Submodule.
Neutralizes Tar Slip vulnerability (CVE-2007-4559).
...
def _validate_archive_member(member: tarfile.TarInfo, dest_resolved: Path) -> None:
    """Validates archive member name, type, and link target to prevent traversal attacks."""
    if "\0" in member.name:
...
def safe_extract_tarball(archive_path: Path, destination_dir: Path) -> None:
    ...
    with tarfile.open(archive_path, "r:*") as tar:
        for member in tar.getmembers():
            ...
            _validate_archive_member(member, dest_resolved)
